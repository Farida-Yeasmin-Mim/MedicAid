<?php
  ...
  if (isset($_POST['search'])) {
    ...
    $sql = "SELECT * FROM plasma WHERE full_name LIKE '%$dbsearch%' OR division LIKE '%$dbsearch%'";
  }
    else
    {
    ...
    if (isset($_POST['block'])) {
      $dbp = $_GET['p'];
      $dbid =  $_GET['id'];
      $sql = "DELETE FROM plasma
      WHERE plasma_id = $dbp";
      mysqli_query($conn, $sql);
      header("location:report_plasma.php?id=$dbid");
    }
  ...
  <?php
    while ($row = mysqli_fetch_array($result)) {
      echo "<div id='img_div'>";
        ...
        echo "<form method='POST' action='report_plasma.php?id=$dbid&p=".$row['plasma_id']."' >";
        echo "<button type='submit' class='btn btn-danger btn-block' name='block'>Block This Person</button>";
        echo "</form>";
