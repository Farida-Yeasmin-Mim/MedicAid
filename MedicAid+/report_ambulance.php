<?php
  ...
  if (isset($_POST['search'])) {
    ...
    $sql = "SELECT * FROM ambulance WHERE owner_name LIKE '%$dbsearch%' OR division LIKE '%$dbsearch%'";
  }
    else
    {
    ...
    if (isset($_POST['block'])) {
      $dbp = $_GET['p'];
      $dbid =  $_GET['id'];
      $sql = "DELETE FROM ambulance
      WHERE ambulance_id = $dbp";
      mysqli_query($conn, $sql);
      header("location:report_ambulance.php?id=$dbid");
    }
    ...
    <?php include 'header2.php';?>
              ...
              <button class="btn bg-primary text-white" onclick="window.location ='admin_profile.php?id=<?php echo $dbid ?>'">Back To Profile</button>
                ...
                <div class="container-fluid">
                  ...
                  <hr class="w-25 pt-5">
              <form class="example" method="POST" action="report_ambulance.php?id=<?php echo $dbid ?>">
  <input name="srch" type="text" placeholder="Search By Name Or Division.." name="search">
  ...
  <?php
    while ($row = mysqli_fetch_array($result)) {
      echo "<div id='img_div'>";
        ...
        echo "<h6><strong>Ambulance Number: </strong>".$row['ambulance_number']." </h6>";
        echo "<h6><strong>Division: </strong>".$row['division']." </h6>";
        echo "<form method='POST' action='report_ambulance.php?id=$dbid&p=".$row['ambulance_id']."' >";
